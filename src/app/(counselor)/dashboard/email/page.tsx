...
export default async function CounselorEmailPage() {
  ...
  const sharedConfig = await getCounselorEmailConfig()
  const canUseEmail = Boolean(emailAccount || sharedConfig)
    ...
    <main className="flex-1 p-4 md:p-6">
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-white/60">
          {emailAccount?.email_address ?? sharedConfig?.user ?? 'No email account configured'}
        </p>
      </div>
