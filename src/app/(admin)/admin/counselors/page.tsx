...
export default async function AdminCounselorsPage() {
  ...
  return (
    <main className="flex-1 p-4 md:p-8">
      <h1 className="text-2xl font-semibold text-blue md:text-3xl">Counselors</h1>
      <p className="mt-1 text-sm text-text/60">
        {counselors.length} active counselor{counselors.length === 1 ? '' : 's'}
      </p>

      {counselors.length === 0 ? (
        <p className="mt-8 text-text/60">No active counselors found.</p>
      ) : (
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
          {counselors.map((counselor) => (
            <article
              key={counselor.id}
              className="flex flex-col rounded-2xl border border-text/10 bg-white p-5"
            >
              <h2 className="text-lg font-bold text-text">{counselor.name}</h2>
              ...
              <Link
                ...
                className="mt-5 inline-flex min-h-[44px] items-center justify-center gap-2 rounded-full bg-blue px-4 py-2.5 text-sm font-semibold text-white transition-opacity hover:opacity-90"
              >
                View Dashboard
